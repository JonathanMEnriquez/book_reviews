...
class AuthorManager(models.Manager):
    def validateAuthor(self, postData):
        if len(postData['newname']) < 3:
            return False
        if not postData['newname'].replace(' ', '').isalpha():
            return False
        else:
            return True
    def addAuthor(self, postData):
        check = Author.objects.filter(name = postData['newname'])
        if len(check) > 0:
            author = Author.objects.get(name = postData['newname'])
            return author
        else:
            author = Author.objects.create(name = postData['newname'])
        return author
...
class BookManager(models.Manager):
    def addBook(self, postData, author):
        check = Book.objects.filter(title = postData['title'])
        if len(check) > 0:
            book = Book.objects.get(title = postData['title'])
            return book
        book = Book.objects.create(title = postData['title'], main_author = author)
        return book


class Book(models.Model):
    title = models.CharField(max_length = 255)
    main_author = models.ForeignKey(Author, related_name='books')
    objects = BookManager()
    created_at = models.DateTimeField(auto_now_add = True)
    updated_at = models.DateTimeField(auto_now = True)
    def __str__(self):
...
class ReviewManager(models.Manager):
    def addRating(self, postData, user, book):
        review = Review.objects.create(rating = postData['rating'], content = postData['content'], book = book, reviewer = user)
        return review
...
class Review(models.Model):
    ...
    reviewer = models.ForeignKey(User, related_name='reviews_by')
    objects = ReviewManager()
    created_at = models.DateTimeField(auto_now_add = True)
    updated_at = models.DateTimeField(auto_now = True)
    def __str__(self):
        return "Review rating: {}, content: {}, book: {}, reviewer: {}".format(self.rating, self.content, self.book, self.reviewer)